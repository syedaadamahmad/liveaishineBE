...
# Intent Detector - Hybrid Regex + Semantic Fallback
# Handles greetings and continuation via deterministic patterns.
# Falls back to Gemini for ambiguous cases ONLY when absolutely necessary.
# """
# import re
# import logging
# from typing import Dict, Any, Optional
...
#         # 3. Semantic fallback ONLY for ambiguous 1-2 word messages (optional)
#         # CRITICAL FIX: Only use semantic for VERY short, ambiguous messages
#         word_count = len(message.split())
#         if self.use_semantic_fallback and word_count <= 2 and message.lower() not in ['what', 'how', 'why', 'when', 'where']:
#             logger.info(f"[INTENT] Ambiguous short message ({word_count} words), using semantic fallback")
#             semantic_intent = self._semantic_detect(message, chat_history)
#             if semantic_intent:
#                 return semantic_intent
...
#         """
#         Semantic intent detection using Gemini (fallback only).
#         Used for ambiguous short messages like "more?", "and?".
